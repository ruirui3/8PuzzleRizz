...
public class Board {
    ...
    // 3 4 5
    // 6 7 8
    // only one of them is sussy imposter (blank)
    public Iterable<Board> neighbors() {
        Stack<Board> neighborsStack = new Stack<>();
        int indexBlank = findIndexBlank();
        int row = indexBlank / bLength;
        int col = indexBlank % bLength;
        //check if top exist, push swap betw blank and up
        if (row > 0) {
            neighborsStack.push(new Board(null))
        }
    ...
    private int findIndexBlank() {
        for (int i = 0; i < board.length; i++) {
            if (board[i].value == 0) {
                return getIndex(board[i].row, board[i].col);
            }
        }
        return -1;
    }
